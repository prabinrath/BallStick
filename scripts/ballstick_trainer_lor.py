...
if not train:
    ...
    try:
        ...
        sys.exit(1)
    try:
    	model = joblib.load('../weights/ballstick_lor.h5')
    	while True:
            buff = s.recv(1024)
            print(buff)
            if(buff=='CONN_CHCK'):
	...
	model = LogisticRegression(random_state=0, solver='lbfgs',multi_class='multinomial').fit(testx, testy.flatten())
	print(model.score(testx,testy))
	joblib.dump(model, '../weights/ballstick_lor.h5')
	# model = joblib.load('../weights/ballstick_lor.h5')
